...
class IWSLT14(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, source_files, target_files, split):
        ...
        def _remove_tags(line, tag):
            # cf.) https://github.com/mjpost/sacrebleu/blob/master/sacrebleu/utils.py
            line = re.sub(r"<" + tag + "[^>]*?>(.*)</" + tag + ">.*?", "\\1", line)
            return line
        ...
        def _get_segments(source_row, target_row, tag):
            # remove tags
            source_row = _remove_tags(source_row.strip(), tag)
            target_row = _remove_tags(target_row.strip(), tag)

            # cleanup
            source_row = _clean(source_row)
            target_row = _clean(target_row)
            ...
            with open(source_file, "r", encoding="utf-8") as sf:
                with open(target_file, "r", encoding="utf-8") as tf:
                    for source_row, target_row in zip(sf, tf):
                        ...
                        # take sents in
                        # - <seg> for test and dev
                        # - <seg>, <title>, and <description> for train
                        # cf.) https://github.com/pytorch/fairseq/blob/main/examples/translation/prepare-iwslt14.sh

                        if source_row.startswith("<"):
                            if source_row.startswith("<seg"):
                                # Remove <seg id="1">.....</seg>
                                source_row, target_row = _get_segments(source_row, target_row, "seg")
                            elif split == "train" and source_row.startswith("<title"):
                                # Remove <title>.....</title>
                                source_row, target_row = _get_segments(source_row, target_row, "title")
                            elif split == "train" and source_row.startswith("<description"):
                                # Remove <description>.....</description>
                                source_row, target_row = _get_segments(source_row, target_row, "description")
                            else:
                                continue
